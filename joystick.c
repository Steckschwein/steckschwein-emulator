...
//					joystick2_data = (joy2_mode != NONE) ? 0 : 1;
					joystick1_data = (joy1_mode == NONE);
					joystick2_data = (joy2_mode == NONE);
				}
			}
		}
	}
...
uint16_t get_joystick_state(SDL_Joystick *control, enum joy_status mode) {
	if (mode == NES) {
		bool a_pressed = SDL_JoystickGetButton(control, SDL_CONTROLLER_BUTTON_A);
	...
	if (mode == SNES) {
		...
		return (!b_pressed) | (!y_pressed) << 1 | (!select_pressed) << 2 | (!start_pressed) << 3 | (!up_pressed) << 4
				...
				| (!x_pressed) << 9 | (!l_pressed) << 10 | (!r_pressed) << 11 | 0xF000;
	}
