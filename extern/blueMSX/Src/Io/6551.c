...
static void acia_update_params( MOS6551 *acia )
{
  ...
  acia->rx = (acia->regs[ACIA_COMMAND] & ACOMF_DTR)? true : false;
  acia->irqrx = (acia->rx && 0 == ( acia->regs[ACIA_COMMAND] & ACOMF_IRQDIS ))? true : false;
  ...
  acia->echo = (acia->regs[ACIA_COMMAND]&ACOMF_ECHO)? true : false;
...
{
#if DEBUG_ACIA
  dbg_printf( "ACIA write: (%04X) %02X to %s", acia->cpu->regs.PC-1, data, writereg_names[addr&3] );
#endif
  switch( addr & 3 )
  {
...
bool acia_init_none( MOS6551 *acia )
{
  ...
  acia->backendType = ACIA_TYPE_NONE;
  return true;
}
